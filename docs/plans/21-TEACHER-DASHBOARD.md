...
In diesem Schritt baust du das Lehrer-Dashboard unter `/teacher`. Dieses Feature ist exklusiv fuer User mit Teacher Premium. Lehrer koennen Klassen erstellen, Schueler hinzufuegen, Spiele als Aufgaben zuweisen und den Fortschritt ihrer Schueler verfolgen. Fuer den MVP wird die Seite mit Mock-Daten aufgebaut.
...
- Datei `20-MINDCOINS-PREMIUM.md` muss abgeschlossen sein (Teacher Premium Check)
- User muss Teacher Premium haben (sonst Redirect zu `/premium`)
...
│  ┌──────────────────────────────────────────────────────────────┐ │
│  │  Schnell-Statistiken                                         │ │
│  │  [2 Klassen]  [24 Schueler]  [5 Aufgaben]  [78% Abschluss] │ │
│  └──────────────────────────────────────────────────────────────┘ │
│                                                                    │
│  ┌──────────────┬──────────────────────────────────────────────┐ │
│  │  Meine       │                                              │ │
│  │  Klassen     │  Klasse: Mathe 10a                           │ │
│  │              │                                              │ │
│  │  ┌────────┐  │  Schueler: 12                                │ │
│  │  │Mathe   │  │  Aufgaben: 3 aktiv                           │ │
│  │  │10a     │  │                                              │ │
│  │  │12 👤   │  │  ┌──────────────────────────────────────┐   │ │
│  │  └────────┘  │  │  Aufgabe: Mathe-Quiz Pro             │   │ │
│  │              │  │  Deadline: 15.02.2025                  │   │ │
│  │  ┌────────┐  │  │  Fortschritt: ████████░░░ 8/12 (67%) │   │ │
│  │  │Physik  │  │  └──────────────────────────────────────┘   │ │
│  │  │9b      │  │                                              │ │
│  │  │12 👤   │  │  ┌──────────────────────────────────────┐   │ │
│  │  └────────┘  │  │  Aufgabe: Geometrie-Runde             │   │ │
│  │              │  │  Deadline: 20.02.2025                  │   │ │
│  │  [+ Klasse]  │  │  Fortschritt: ████░░░░░░░ 5/12 (42%) │   │ │
│  │              │  └──────────────────────────────────────┘   │ │
│  └──────────────┴──────────────────────────────────────────────┘ │
│                                                                    │
│  [ + Aufgabe zuweisen ]                                          │
...
## Teacher Premium Guard

Jeder Zugriff auf `/teacher` muss pruefen ob der User Teacher Premium hat:
    ...
    assignments: [
      {
        ...
        deadline: new Date(Date.now() + 9 * 24 * 60 * 60 * 1000),  // in 9 Tagen
      ...
      },
      {
        ...
        deadline: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000),  // in 14 Tagen
  ...
  classes: [
    {
      id: 'class-1',
      name: 'Mathe 10a',
      description: 'Mathematik Klasse 10a',
...
### Assignment-Completions (Sub-Collection)

```javascript
// Collection: teachers/{teacherUid}/assignments/{assignmentId}/completions
// Dokument-ID: student UID
{
  studentId: 'student-1',
  completed: true,
  completedAt: Timestamp,
  score: 85,  // Optional: Punktzahl
}
```

---

## Datei 1: `src/pages/TeacherDashboard.jsx`

Die Haupt-Dashboard-Seite.
...
function QuickStats({ classes }) {
  ...
  // Durchschnittliche Abschlussrate berechnen
  ...
  const stats = [
    { label: 'Klassen', value: classes.length, icon: '🏫' },
    { label: 'Schueler', value: totalStudents, icon: '👤' },
    { label: 'Aufgaben', value: totalAssignments, icon: '📋' },
...
## Schueler-Enrollment (Basic fuer MVP)

Fuer den MVP kann ein einfaches System verwendet werden:

```jsx
// Option 1: Klassen-Code (einfachste Variante)
// Lehrer generiert einen Code, Schueler geben ihn ein
// Code-Format: 6 Zeichen alphanumerisch (z.B. "XK3M9P")

// Option 2: Username-Suche (wie im Plan beschrieben)
// Lehrer sucht Schueler ueber Username und fuegt sie hinzu

// Fuer den MVP reicht Option 1 als Platzhalter mit Hinweis
...
1. **Zugriffs-Schutz** - Ohne Teacher Premium: Redirect/Hinweis zu `/premium`
2. **Dashboard laedt** - Mit Teacher Premium: Dashboard zeigt Schnell-Statistiken
3. **Klassen-Sidebar** - 2 Mock-Klassen werden angezeigt
4. **Klasse waehlen** - Klick auf Klasse zeigt Details rechts
5. **Aufgaben angezeigt** - Zugewiesene Spiele mit Deadline und Fortschritt
6. **Fortschrittsbalken** - Korrekte Berechnung (z.B. 8/12 = 67%)
7. **Farbcodierung** - Gruen >80%, Gelb 50-80%, Rot <50%
8. **Deadline-Anzeige** - Verbleibende Tage und Ueberfaellig-Warnung
9. **Klasse erstellen** - Modal oeffnet sich, Name eingeben, Klasse erscheint in Sidebar
10. **Aufgabe zuweisen** - Modal oeffnet sich, Spiel und Deadline waehlen
11. **Responsive** - Layout passt sich an (Sidebar oben auf Mobile)

---

## Checkliste

- [ ] Teacher Dashboard unter `/teacher` ist erreichbar
- [ ] Zugriffs-Schutz: Nur Teacher Premium User koennen zugreifen
- [ ] Nicht-Premium User sehen Hinweis mit Link zu `/premium`
- [ ] Schnell-Statistiken: Klassen, Schueler, Aufgaben, Abschlussrate
- [ ] Klassen-Sidebar zeigt alle Klassen des Lehrers
- [ ] Klassen-Auswahl zeigt Details im Hauptbereich
- [ ] AssignmentCard zeigt: Spielname, Deadline, Fortschrittsbalken
- [ ] Fortschrittsbalken mit Farbcodierung (gruen/gelb/rot)
- [ ] Deadline-Countdown mit Ueberfaellig-Warnung
- [ ] CreateClassModal: Name und Beschreibung eingeben
- [ ] AssignGameModal: Spiel auswaehlen und Deadline setzen
- [ ] Mock-Daten: 2 Klassen mit je 12 Schuelern und Aufgaben
- [ ] Neue Klasse erstellen funktioniert (lokaler State)
- [ ] Neue Aufgabe zuweisen funktioniert (lokaler State)
- [ ] Responsive Layout
...
| Datei | Zweck |
|-------|-------|
| `src/pages/TeacherDashboard.jsx` | Teacher Dashboard mit Klassen-Verwaltung und Aufgaben-Tracking |
| `src/components/teacher/CreateClassModal.jsx` | Modal zum Erstellen neuer Klassen (optional separat) |
| `src/components/teacher/AssignGameModal.jsx` | Modal zum Zuweisen von Spielen als Aufgaben (optional separat) |
